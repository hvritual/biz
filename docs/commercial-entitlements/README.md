...
CE-01～CE-13 和 CE-16 已完成并在 `tasks.json` 标为 `DONE`；其余任务仍按台账的依赖和外部条件推进。完成表示相应任务的实现、验证、main 集成和回执已处理，不等同于生产发布或未完成路线的自动交付。

当前企业页面保持完整的模拟数据体验：所有已展示的布局、导航、表格、操作和状态都应在受控 demo 数据中可用，并按租户隔离保存。真实 API 模式尚未接入这些当前页面；不得把接口失败伪装为模拟成功，也不得因尚未接入 API 而将页面简化为空白、静态截图或不完整界面。正式接入时仍须使用服务端可信会话与授权，不向浏览器暴露 API key。
