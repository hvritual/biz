...
func ce05LegacyFixtureGrants(t *testing.T,db *gorm.DB,tenant string,modules []string) {
 t.Helper()
 store,err:=modulecatalog.NewStore(db);if err!=nil {t.Fatal(err)}
 svc,err:=modulecatalog.NewService(store,modulecatalog.ProductionRegistry());if err!=nil {t.Fatal(err)}
 ...
 for _,code:=range modules {
  ...
  if errors.Is(err,modulecatalog.ErrNotFound) { _,err=svc.Create(ctx,modulecatalog.CreateCommand{Code:code,Name:code,Reason:"legacy integration fixture",RequestID:ce04Random(t)}) }
  if err!=nil {t.Fatal(err)}
 }
 err=db.Transaction(func(tx *gorm.DB)error {
  if err:=tx.Exec("INSERT IGNORE INTO biz_commercial_entitlement_state(tenant_id,version) VALUES(?,1)",tenant).Error;err!=nil{return err}
  for _,code:=range modules {
   ...
   if err:=tx.Exec("INSERT IGNORE INTO biz_commercial_entitlement_sources(tenant_id,source_id,module_code,version,payload) VALUES(?,?,?,?,?)",tenant,src.ID,code,1,string(b)).Error;err!=nil{return err}
  }
  return nil
 });if err!=nil {t.Fatal(err)}
}

type ce05Environment struct { *ce04Environment; devices devicev1.DeviceApplicationClient; transfers devicev1.DeviceTransferApplicationClient; siteA,siteB string }
func ce05New(t *testing.T) *ce05Environment {
 t.Helper();e:=ce04NewEnvironment(t,"")
 ...
 access,err:=accesspersistence.New(e.db);if err!=nil {t.Fatal(err)}
 sites,err:=devicepersistence.NewSiteRepository(e.db);if err!=nil{t.Fatal(err)}
 for _,v:=range []struct{tenant,token string}{{e.tenantA,e.tokenA},{e.tenantB,e.tokenB}} {
  for _,permission:=range []string{"device.read","device.create","device.update","device.delete","site.read"} {
   if err:=e.db.Exec("INSERT IGNORE INTO biz_permission_grants(tenant_id,role_id,permission,scope) SELECT tenant_id,role_id,?,'all' FROM biz_member_roles WHERE tenant_id=? AND user_id=?",permission,v.tenant,v.tenant+"-user").Error;err!=nil{t.Fatal(err)}
  }
  p,err:=access.Authenticate(context.Background(),v.token);if err!=nil{t.Fatal(err)}
  ctx:=identity.WithPrincipal(context.Background(),p)
  for _,id:=range []string{out.siteA,out.siteB} { if err:=sites.Create(ctx,&domain.Site{ID:id,Name:id});err!=nil{t.Fatal(err)} }
 }
 return out
}
