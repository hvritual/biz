...
func TestCE06PersistenceBeforeRestart(t *testing.T) {
	...
	view := e.explain(e.tenantA)
	if view.EntitlementVersion != 1 || view.SourceVersion != 1 {
		t.Fatal(view)
	}
...
func TestCE06PersistenceAfterRestart(t *testing.T) {
	e := ce04NewEnvironment(t, "ce06-restart")
	view := e.explain(e.tenantA)
	if view.EntitlementVersion != 1 || view.SourceVersion != 1 || !ce04Decision(t, view, "capability", "device.lifecycle", "").Allowed {
		t.Fatal("immutable snapshot did not survive", view)
	}
