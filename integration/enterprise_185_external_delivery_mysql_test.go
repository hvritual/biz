...
func TestEnterprise185ExternalDeliveryRechecksPreferenceAndProtectedContact(t *testing.T) {
	...
	if err := store.Bootstrap(ctx, accesspersistence.Bootstrap{TenantID:tenant,TenantName:tenant,UserID:user,Email:email,Token:"n185-token-"+stamp}, []authz.PermissionKey{"tenant.notification.read"}); err != nil { t.Fatal(err) }
...
VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
			...
			"trace-"+id,"device","machine-"+stamp,domain.ExternalTaskStatePending,now,now).Error; err != nil { t.Fatal(err) }
	}
	forceDue := func(id string) {
		t.Helper()
		if err := db.Exec("UPDATE biz_notification_external_tasks SET next_attempt_at=DATE_SUB(UTC_TIMESTAMP(6), INTERVAL 1 SECOND) WHERE task_id=?", id).Error; err != nil { t.Fatal(err) }
	}

	t.Run("preference-deny-after-enqueue-cancels-before-provider", func(t *testing.T) {
		id := "task-deny-" + stamp
		insertTask(id, "email")
		if _, err := store.ChangeNotificationPreference(ctx, owner, accessdomain.NotificationPreferenceChange{Channel:accessdomain.NotificationPreferenceEmail,Allowed:false,ExpectedVersion:0,IdempotencyKey:"deny-"+stamp}); err != nil { t.Fatal(err) }
	...
	t.Run("allowed-delivery-resolves-protected-contact-transiently", func(t *testing.T) {
		if _, err := store.ChangeNotificationPreference(ctx, owner, accessdomain.NotificationPreferenceChange{Channel:accessdomain.NotificationPreferenceEmail,Allowed:true,ExpectedVersion:1,IdempotencyKey:"allow-"+stamp}); err != nil { t.Fatal(err) }
		id := "task-allow-" + stamp
		insertTask(id, "email")
		receipt, err := worker.RunOnce(ctx)
		...
		if err := db.Table("biz_memberships").Select("email,email_ciphertext").Where("tenant_id=? AND user_id=?",tenant,user).Take(&member).Error; err != nil { t.Fatal(err) }
		...
		var taskDump string
		if err := db.Raw(`SELECT CONCAT_WS('|',task_id,tenant_id,user_id,event_id,channel,configuration_id,group_id,type_code,trace_id,reference_kind,reference_id,state,failure_code,provider_receipt) FROM biz_notification_external_tasks WHERE task_id=?`,id).Scan(&taskDump).Error; err != nil { t.Fatal(err) }
	...
	t.Run("retry-rechecks-new-deny-before-second-provider-call", func(t *testing.T) {
		if _, err := store.ChangeNotificationPreference(ctx, owner, accessdomain.NotificationPreferenceChange{Channel:accessdomain.NotificationPreferenceEmail,Allowed:true,ExpectedVersion:2,IdempotencyKey:"allow-retry-"+stamp}); err != nil { t.Fatal(err) }
		id := "task-retry-" + stamp
		insertTask(id, "email")
		...
		if _, err := store.ChangeNotificationPreference(ctx, owner, accessdomain.NotificationPreferenceChange{Channel:accessdomain.NotificationPreferenceEmail,Allowed:false,ExpectedVersion:3,IdempotencyKey:"deny-retry-"+stamp}); err != nil { t.Fatal(err) }
		forceDue(id)
