...
const qualifiedYunkaRevision = "6ba99c1440dc6c9416f6afd08f3282e35fa5a3fb"
...
func TestCE01(t *testing.T) {
	t.Run("qualified framework revision remains pinned", func(t *testing.T) {
		...
		text := string(content)
		if !strings.Contains(text, "yunka.io/framework v0.0.0-20260902030847-6ba99c1440dc") ||
			!strings.Contains(text, "yunka.io/gateway v0.0.0-20260902030847-6ba99c1440dc") ||
			!strings.Contains(text, "yunka.io/pkg v0.0.0-20260902030847-6ba99c1440dc") {
	...
	t.Run("generated operation plan is the traceable operation inventory", func(t *testing.T) {
		...
		if plan.SchemaVersion != 2 { t.Fatalf("CE01-OPERATIONS: schemaVersion=%d, want 2", plan.SchemaVersion) }
		if len(plan.Operations) == 0 { t.Fatal("CE01-OPERATIONS: generated operation inventory is empty") }
		seen := map[string]bool{}
		for _, operation := range plan.Operations {
			id := strings.TrimSpace(operation.OperationID)
			if id == "" { t.Fatal("CE01-OPERATIONS: blank operationId") }
			if seen[id] { t.Fatalf("CE01-OPERATIONS: duplicate operationId %q", id) }
			seen[id] = true
		}
		for _, required := range []string{"device.create", "device.transfer", "site.validate_transfer_target", "tenant.create", "tenant.member.bootstrap_owner", "tenant.role.bootstrap_owner"} {
			if !seen[required] { t.Fatalf("CE01-OPERATIONS: required baseline operation %q disappeared", required) }
		}
	})
	...
	t.Run("device transfer composition remains explicit", func(t *testing.T) {
		...
		for _, operation := range plan.Operations {
			if operation.OperationID == "device.transfer" {
				if !operation.Security.TenantRequired { t.Fatal("CE01-CHILD: device.transfer must remain tenant-required") }
	...
	t.Run("internal child operation stays transport private", func(t *testing.T) {
		plan := loadCE01OperationPlans(t)
		for _, operation := range plan.Operations {
			if operation.OperationID != "site.validate_transfer_target" { continue }
...
func loadCE01OperationPlans(t *testing.T) ce01OperationPlans {
	...
	if err := json.Unmarshal(content, &result); err != nil { t.Fatal(err) }
	return result
}
