...
func fromCreate(req *commercialv1.CreateEntitlementOverrideRequest, actor string, at time.Time) (entitlement.Source, error) {
	kind, ok := kinds[req.Target]
	if !ok {
		return entitlement.Source{}, entitlement.ErrInvalid
	}
	effect, ok := effects[req.Effect]
	if !ok {
		return entitlement.Source{}, entitlement.ErrInvalid
	}
	start := at
	if req.EffectiveAt != "" {
		v, err := time.Parse(time.RFC3339Nano, req.EffectiveAt)
		if err != nil {
			return entitlement.Source{}, entitlement.ErrInvalid
		}
		start = v.UTC()
	...
	var end *time.Time
	if req.ExpiresAt != "" {
		v, err := time.Parse(time.RFC3339Nano, req.ExpiresAt)
		if err != nil {
			return entitlement.Source{}, entitlement.ErrInvalid
		}
		v = v.UTC()
	...
	if req.Limit != nil {
		limit = entitlement.Limit{Unlimited: req.Limit.Unlimited, Value: req.Limit.Value}
	} else if kind == entitlement.Quota {
		return entitlement.Source{}, entitlement.ErrInvalid
	}
	id, err := newID()
	if err != nil {
		return entitlement.Source{}, err
	}
...
func instant(v *time.Time) string {
	if v == nil {
		return ""
	}
...
func receiptDTO(r ports.OverrideReceipt) *commercialv1.EntitlementOverrideReceipt {
...
func resultDTO(r entitlement.Result, redact bool) *commercialv1.EntitlementView {
	...
	for _, d := range r.Decisions {
		...
		for _, s := range d.Sources {
			e := &commercialv1.EntitlementSourceExplanation{Id: s.ID, SourceKind: string(s.Kind), Effect: string(s.Effect), State: s.State, Disposition: s.Disposition}
			if !redact {
				e.Reason = s.Reason
				e.ActorId = s.ActorID
			}
			entry.Sources = append(entry.Sources, e)
		}
