...
type decisionReader struct {
	sources ports.EntitlementStateReader
	catalog ports.EntitlementCatalogReader
}
...
func BuildDecisionReader(sources ports.EntitlementStateReader, catalog ports.EntitlementCatalogReader) (ports.EntitlementDecisionReader, error) {
	if sources == nil || catalog == nil {
		return nil, errors.New("commercial: source and catalog readers required")
	}
...
func (r *decisionReader) Decide(ctx context.Context, requested []string) (entitlement.Result, error) {
	...
	catalog, err := r.catalog.ReadCurrentCatalog(ctx)
	if err != nil {
		return entitlement.Result{}, err
	}
	state, err := r.sources.ReadCurrent(ctx, p.TenantID)
	if err != nil {
		return entitlement.Result{}, err
	}
