...
func (s *service) preparationRequirements(ctx context.Context, action string, target plan.Version) ([]pv.Requirement, error) {
	if action == change.StopRenewal {
		return nil, nil
	}
	rs, err := s.provisioningPolicy.Requirements(ctx, target)
	if err != nil {
		return nil, err
	}
	if err = pv.ValidateRequirements(rs); err != nil {
		return nil, err
	}
	if len(rs) == 0 {
		return nil, nil
	...
	for _, src := range sources {
		if previous, ok := existing[src.ID]; ok {
			if src.SourceKind == entitlement.PlanSource && previous.RevokedAt == nil && src.RevokedAt != nil {
				if err = repos.Entitlements.Revoke(ctx, src, previous.Version); err != nil {
					return 0, 0, err
				}
			}
		} else {
			if err = src.Validate(m.catalog); err != nil {
				return 0, 0, err
			}
			if err = repos.Entitlements.Insert(ctx, src); err != nil {
				return 0, 0, err
			}
		}
	}
