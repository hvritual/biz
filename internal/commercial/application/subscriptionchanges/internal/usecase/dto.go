...
func quotaDTO(q change.QuotaImpact) *v1.SubscriptionChangeQuotaImpact {
	return &v1.SubscriptionChangeQuotaImpact{ModuleCode: q.ModuleCode, Key: q.Key, BeforeLimit: &v1.EntitlementLimit{Unlimited: q.Before.Unlimited, Value: q.Before.Value}, AfterLimit: &v1.EntitlementLimit{Unlimited: q.After.Unlimited, Value: q.After.Value}, UsageKnown: q.UsageKnown, Used: q.Used, OverLimit: q.OverLimit, Policy: q.Policy, Evidence: q.Evidence}
}
func previewDTO(p change.Preview) *v1.SubscriptionChangePreviewDTO {
	...
	for _, d := range p.Dependencies {
		out.Dependencies = append(out.Dependencies, &v1.SubscriptionChangeDependency{ModuleCode: d.ModuleCode, RequiresModules: append([]string(nil), d.Requires...)})
	}
