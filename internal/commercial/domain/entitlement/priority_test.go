...
func TestCE04ExplicitDenialWithoutUsableGrant(t *testing.T) {
	...
	if decision := findDecision(t, result, Field, "device.identity", "read"); decision.Allowed || decision.Reason != "FIELD_DISABLED" {
		t.Fatal(decision)
	}
	c[0].TechnicalStatus = "disabled"
	result = resolveFixture(t, epoch, c, []Source{field, denied})
	if decision := findDecision(t, result, Capability, "devices.read", ""); decision.Reason != "TECHNICAL_UNAVAILABLE" {
		t.Fatal(decision)
	}
	if decision := findDecision(t, result, Field, "device.identity", "read"); decision.Reason != "TECHNICAL_UNAVAILABLE" {
		t.Fatal(decision)
	}
}
