...
func (e Event) Integrity() error {
	sealed := e.Seal()
	if !Tenant(e.TenantID) || !Key(e.AggregateID) || e.AggregateVersion == 0 || !Key(e.ChangeID) || e.OccurredAt.IsZero() || e.SourceVersion == 0 || e.EntitlementVersion == 0 || sealed.ID != e.ID || sealed.Hash != e.Hash {
		return ErrCorrupt
	}
	switch e.Status {
	case Applied, "SCHEDULED", "PROVISIONING", Cancelled:
	default:
		return ErrCorrupt
	}
	return nil
