...
func (r Requirement) Validate() error {
	if !Key(r.Code) || !Key(r.Adapter) || !Key(r.Version) || r.MaxAttempts < 1 || r.MaxAttempts > 10 {
		return ErrInvalid
	}
	return nil
}
...
func New(tenant string, a Approval, rs []Requirement, now time.Time) (Task, error) {
	...
	if len(rs) == 0 || ValidateRequirements(rs) != nil {
		return t, ErrInvalid
	}
	for i, r := range rs {
		t.Steps = append(t.Steps, Step{Requirement: r, IdempotencyKey: "prep-" + Digest([]any{t.ID, i, r})[:48], State: WaitingStep, Effect: Absent})
	}
	t = t.Seal()
...
func (t Task) Integrity() error {
	if !Tenant(t.TenantID) || t.ID != TaskID(t.Approval.ChangeID) || !Key(t.Approval.ChangeID) || t.Revision == 0 || t.Approval.ActorID == "" || len(t.Approval.PreviewHash) != 64 || len(t.Approval.TargetHash) != 64 || !Key(t.Approval.TargetPlanCode) || t.Approval.TargetPlanVersion == 0 || t.Approval.SubscriptionRevision == 0 || t.Approval.SourceVersion == 0 || t.Approval.EntitlementVersion == 0 || t.Approval.CatalogRevision == 0 || len(t.Steps) < 1 || len(t.Steps) > 16 || t.StepIndex < 0 || t.StepIndex > len(t.Steps) || t.CreatedAt.IsZero() || !t.Deadline.After(t.CreatedAt) || t.Hash != t.Seal().Hash {
		return ErrCorrupt
	}
	switch t.State {
	case Queued, Running, RetryWait, Ready, Applied, Failed, ReconcileRequired, Cancelled:
	default:
		return ErrCorrupt
	}
	for i, s := range t.Steps {
		if s.Requirement.Validate() != nil || s.IdempotencyKey != "prep-"+Digest([]any{t.ID, i, s.Requirement})[:48] || s.CycleAttempts > s.Attempts || s.CycleAttempts > s.Requirement.MaxAttempts || len(s.Evidence) > 1024 || len(s.FailureCode) > 128 {
			return ErrCorrupt
		}
		switch s.State {
		case WaitingStep, RunningStep, ReadyStep:
		default:
			return ErrCorrupt
		}
		switch s.Effect {
		case Absent, Unknown, ReadyStep:
		default:
			return ErrCorrupt
		}
		if i < t.StepIndex && (s.State != ReadyStep || s.Effect != ReadyStep || s.Evidence == "") {
			return ErrCorrupt
		}
	}
	if t.State == Running && (t.LeaseOwner == "" || t.LeaseUntil == nil || t.LeaseToken == 0) {
		return ErrCorrupt
	}
	if t.State != Running && (t.LeaseOwner != "" || t.LeaseUntil != nil) {
		return ErrCorrupt
	}
	if t.State == Applied && (t.Completion == nil || t.Completion.ChangeID != t.Approval.ChangeID || t.Completion.EntitlementVersion == 0 || t.Completion.SourceVersion == 0 || t.Completion.SubscriptionRevision != t.Approval.SubscriptionRevision+1) {
		return ErrCorrupt
	}
	return nil
}
...
func (t *Task) Claim(owner string, now time.Time, lease time.Duration) error {
	...
	if !now.Before(t.Deadline) {
		t.State = ReconcileRequired
		t.FailureCode = "APPROVAL_DEADLINE_EXCEEDED"
		t.RetryAllowed = false
		t.release()
		t.finishRevision(now)
		return nil
	}
	expired := t.State == Running
	if t.StepIndex == len(t.Steps) {
		t.Stage = ActivateStage
		t.ActivationAttempts++
		if t.ActivationAttempts > 3 {
			t.State = Failed
			t.FailureCode = "ACTIVATION_RETRIES_EXHAUSTED"
			t.RetryAllowed = true
			t.release()
			t.finishRevision(now)
			return nil
		...
		if expired || s.Effect == Unknown {
			if s.Reconciliations >= 32 {
				t.State = ReconcileRequired
				t.FailureCode = "RECONCILIATION_LIMIT_REACHED"
				t.RetryAllowed = false
				t.release()
				t.finishRevision(now)
				return nil
			}
			t.Stage = ReconcileStage
			s.Reconciliations++
		} else {
			if s.CycleAttempts >= s.Requirement.MaxAttempts {
				t.State = Failed
				t.FailureCode = "PREPARATION_RETRIES_EXHAUSTED"
				t.RetryAllowed = true
				t.release()
				t.finishRevision(now)
				return nil
			}
			t.Stage = PrepareStage
			s.Attempts++
			s.CycleAttempts++
		}
		s.State = RunningStep
...
func (o Observation) Validate() error {
	if len(o.Evidence) > 1024 || strings.TrimSpace(o.Evidence) == "" || len(o.FailureCode) > 128 {
		return ErrInvalid
	}
	switch o.Outcome {
	case ReadyStep, Absent, Retryable, Permanent, Unknown:
	default:
		return ErrInvalid
	}
	if o.Outcome != ReadyStep && o.Outcome != Absent && !Key(o.FailureCode) {
		return ErrInvalid
	}
	return nil
}
func Backoff(n uint32) time.Duration {
	if n > 6 {
		n = 6
	}
...
func (t *Task) Observe(owner string, token uint64, o Observation, now time.Time) error {
	if !t.Owns(owner, token, now) {
		return ErrLease
	}
	if o.Validate() != nil {
		return ErrInvalid
	}
	t.FailureCode = o.FailureCode
	t.RetryAllowed = false
	if t.Stage == ActivateStage {
		if o.Outcome == Retryable {
			t.State = RetryWait
			t.NextAttemptAt = now.Add(Backoff(t.ActivationAttempts))
			t.RetryAllowed = true
		} else {
			t.State = ReconcileRequired
		}
	...
	case Absent, Retryable:
		...
		if s.CycleAttempts >= s.Requirement.MaxAttempts {
			...
			t.State = RetryWait
			t.NextAttemptAt = now.Add(Backoff(s.CycleAttempts))
		}
	case Permanent:
		s.State = WaitingStep
		s.Effect = Absent
...
func (t Task) Cancellable() bool {
	...
	for _, s := range t.Steps {
		if s.Effect != Absent || s.State == ReadyStep {
			return false
		}
	}
	return true
...
func (t *Task) Retry(now time.Time) error {
	if (t.State != Failed && t.State != ReconcileRequired) || !t.RetryAllowed || !now.Before(t.Deadline) || t.RetryCycles >= 10 {
		return ErrRetry
	}
	t.RetryCycles++
	t.ActivationAttempts = 0
	if t.StepIndex < len(t.Steps) {
		t.Steps[t.StepIndex].CycleAttempts = 0
		t.Steps[t.StepIndex].State = WaitingStep
	}
	t.State = Queued
	t.NextAttemptAt = now
	t.RetryAllowed = false
...
func (t *Task) Complete(owner string, token uint64, c Completion, now time.Time) error {
	if !t.Owns(owner, token, now) || t.Stage != ActivateStage || t.StepIndex != len(t.Steps) {
		return ErrLease
	}
	if c.ChangeID != t.Approval.ChangeID || c.SubscriptionRevision != t.Approval.SubscriptionRevision+1 || c.SourceVersion <= t.Approval.SourceVersion || c.EntitlementVersion <= t.Approval.EntitlementVersion || c.AppliedAt.IsZero() {
		return ErrCorrupt
	}
	t.State = Applied
	t.Completion = &c
