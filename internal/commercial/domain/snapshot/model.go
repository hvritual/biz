...
func ValidAt(r entitlement.Result, at time.Time) bool {
	return !at.Before(r.EvaluatedAt) && (r.ValidUntil == nil || at.Before(*r.ValidUntil))
}
func Decode(b []byte, tenant string, version, source, catalog uint64, digest string, at time.Time) (entitlement.Result, error) {
	if Digest(b) != digest {
		return entitlement.Result{}, ErrInvalid
	}
	var r entitlement.Result
	if err := json.Unmarshal(b, &r); err != nil {
		return r, ErrInvalid
	}
	if r.TenantID != tenant || r.EntitlementVersion != version || version == 0 || r.SourceVersion != source || r.CatalogRevision != catalog || r.ResolverVersion != 1 || r.PermissionVersion != "" || r.PermissionSubject != "" {
		return entitlement.Result{}, ErrInvalid
	}
	if !ValidAt(r, at) {
		return entitlement.Result{}, ErrStale
	}
	return r, nil
}
...
func Requested(r entitlement.Result, requested []string) entitlement.Result {
	...
	for _, d := range r.Decisions {
		if d.Kind == entitlement.Capability {
			seen[d.Key] = true
		}
	}
	for _, key := range requested {
		if !seen[key] {
			r.Decisions = append(r.Decisions, entitlement.Decision{Kind: entitlement.Capability, Key: key, Allowed: false, Reason: "UNKNOWN_CAPABILITY"})
			seen[key] = true
		}
	}
	return r
