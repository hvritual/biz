...
func Sources(tenant,namespace string,at time.Time,terms plan.Terms) []entitlement.Source {
 ...
 for _,m:=range terms.Modules {
  ...
  for _,c:=range m.Capabilities {appendSource(m.Code,entitlement.Capability,c,"",entitlement.Grant,entitlement.Limit{})}
  for _,q:=range m.Quotas {appendSource(m.Code,entitlement.Quota,q.Key,"",entitlement.QuotaReplace,entitlement.Limit{Unlimited:q.Unlimited,Value:q.Value})}
  for _,f:=range m.Fields {effect:=entitlement.Grant;if f.Mode=="deny"{effect=entitlement.Deny}else if f.Mode=="masked"{effect=entitlement.SafetyMask};appendSource(m.Code,entitlement.Field,f.Key,f.Action,effect,entitlement.Limit{})}
 }
 if terms.ValidityMode=="fixed_days" {end:=at.AddDate(0,0,int(terms.ValidityDays));for i:=range out {v:=end;out[i].ExpiresAt=&v}}
 return out
}
