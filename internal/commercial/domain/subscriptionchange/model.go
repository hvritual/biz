...
func Normalize(s subscription.Subscription, p plan.Version) (subscription.Subscription, error) {
	if s.PlanCode != p.PlanCode || s.PlanVersion != p.Number || s.Validate() != nil {
		return s, ErrCorrupt
	}
	if s.Revision == 0 {
		s.Revision = 1
		s.PeriodStart = s.CreatedAt
		if p.Terms.ValidityMode == "fixed_days" {
			end := s.CreatedAt.AddDate(0, 0, int(p.Terms.ValidityDays))
			s.PeriodEnd = &end
		}
	}
	if s.SourceNamespace == "" {
		s.SourceNamespace = s.ID
	}
	if s.PeriodStart.IsZero() || (s.PeriodEnd != nil && !s.PeriodEnd.After(s.PeriodStart)) {
		return s, ErrCorrupt
	}
	return s, nil
}
func Period(i Input, before subscription.Subscription, classification string, now time.Time, target plan.Terms) (string, time.Time, *time.Time, error) {
	...
	if i.EffectiveAt != nil {
		if !i.EffectiveAt.After(now) || i.EffectiveAt.After(now.AddDate(100, 0, 0)) {
			return "", at, nil, ErrInvalid
		}
		mode = Scheduled
		at = i.EffectiveAt.UTC()
	} else if i.Action == Switch && classification == "DOWNGRADE" {
		if before.PeriodEnd == nil {
			return "", at, nil, ErrPeriod
		}
		if before.PeriodEnd.After(now) {
			mode = Scheduled
			at = *before.PeriodEnd
		}
	}
	if i.Action == StopRenewal {
		return Immediate, now, before.PeriodEnd, nil
	}
	var end *time.Time
	if i.Action == Renew {
		if target.ValidityMode != "fixed_days" || before.PeriodEnd == nil {
			return "", at, nil, ErrPeriod
		}
		base := at
		if before.PeriodEnd.After(base) {
			base = *before.PeriodEnd
		}
		v := base.AddDate(0, 0, int(target.ValidityDays))
		end = &v
	} else if target.ValidityMode == "fixed_days" {
		v := at.AddDate(0, 0, int(target.ValidityDays))
		end = &v
	}
...
func Classify(a, b plan.Terms) string {
	features := func(t plan.Terms) map[string]int {
		...
		for _, m := range t.Modules {
			...
			for _, f := range m.Fields {
				rank := map[string]int{"deny": 0, "masked": 1, "allow": 2}[f.Mode]
				out["f/"+m.Code+"/"+f.Key+"/"+f.Action] = rank
			}
		}
		return out
	}
	old, next := features(a), features(b)
	up, down := false, false
	for k, v := range old {
		if next[k] < v {
			down = true
	...
	for _, q := range QuotaChanges(a, b) {
	...
	if a.ValidityMode == "unlimited" && b.ValidityMode != "unlimited" {
		down = true
	}
	if a.ValidityMode != "unlimited" && b.ValidityMode == "unlimited" {
		up = true
	}
	if a.ValidityMode == "fixed_days" && b.ValidityMode == "fixed_days" {
		if b.ValidityDays < a.ValidityDays {
			down = true
		}
		if b.ValidityDays > a.ValidityDays {
			up = true
...
func QuotaChanges(a, b plan.Terms) []QuotaImpact {
	old, next := map[string]entitlement.Limit{}, map[string]entitlement.Limit{}
	keys := map[string]QuotaImpact{}
	add := func(t plan.Terms, values map[string]entitlement.Limit) {
		for _, m := range t.Modules {
			for _, q := range m.Quotas {
				k := m.Code + "/" + q.Key
				values[k] = entitlement.Limit{Unlimited: q.Unlimited, Value: q.Value}
				keys[k] = QuotaImpact{ModuleCode: m.Code, Key: q.Key}
			}
		}
	}
	add(a, old)
