...
func DB(ctx context.Context, fallback *gorm.DB) (*gorm.DB, error) {
	if f, ok := execution.Current(ctx); ok && f.Transaction != execution.TransactionNone {
		h, err := execution.TransactionHandleFrom(ctx)
		if err != nil {
			return nil, err
		}
		db, ok := h.(*gorm.DB)
		if !ok || db == nil {
			return nil, errors.New("commercial: incompatible root transaction")
		}
		return db.WithContext(ctx).Session(&gorm.Session{SkipDefaultTransaction: true}), nil
	}
	if fallback == nil {
		return nil, errors.New("commercial: authoritative database required")
	}
	return fallback.WithContext(ctx), nil
}
func Transient(err error) bool {
	var m *mysql.MySQLError
	return errors.As(err, &m) && (m.Number == 1213 || m.Number == 1205)
}
...
func Within(ctx context.Context, db *gorm.DB, retries int, fn func(*gorm.DB) error) error {
	if f, ok := execution.Current(ctx); ok && f.Transaction != execution.TransactionNone {
		if f.Transaction != execution.TransactionLocal {
			return errors.New("commercial: writable root transaction required")
		}
		tx, err := DB(ctx, nil)
		if err != nil {
			return err
		...
		err = fn(tx)
		if Transient(err) {
			return fmt.Errorf("%w: %w", ErrRetryRequired, err)
		}
		return err
	}
...
func LockCatalog(tx *gorm.DB, exclusive bool) (uint64, error) {
	...
	if errors.Is(err, gorm.ErrRecordNotFound) || (err == nil && state.Version == 0) {
		return 0, ErrStateLost
	}
	return state.Version, err
}
func AdvanceCatalog(tx *gorm.DB) error {
	r := tx.Model(&CatalogState{}).Where("id=1 AND version < ?", ^uint64(0)).Update("version", gorm.Expr("version+1"))
	if r.Error != nil {
		return r.Error
	}
	if r.RowsAffected != 1 {
...
func Now(tx *gorm.DB) (time.Time, error) {
	var v time.Time
	err := tx.Raw("SELECT UTC_TIMESTAMP(6)").Row().Scan(&v)
	return v.UTC(), err
}
