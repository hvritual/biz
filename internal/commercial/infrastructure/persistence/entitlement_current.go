...
func (r *currentEntitlements) ReadCurrent(ctx context.Context, tenant string) (ports.EntitlementState, error) {
	...
	if _, ok := execution.Current(ctx); ok {
		return requestscope.JoinValue(ctx, NewEntitlementRepositoryFactory(), func(scope *requestscope.View[ports.EntitlementRepositories]) (ports.EntitlementState, error) {
 ...
 ON o.tenant_id=s.tenant_id WHERE s.tenant_id=? ORDER BY o.source_id`, tenant).Scan(&rows).Error
	...
	for _, row := range rows {
		...
		if !row.ModuleCode.Valid || !row.Version.Valid || row.Version.Int64 < 1 || !row.Payload.Valid {
			return ports.EntitlementState{}, entitlement.ErrInvalid
		}
