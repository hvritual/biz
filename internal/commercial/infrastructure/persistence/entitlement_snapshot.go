...
type SnapshotCatalogProjection interface{ReadSnapshotCatalog(context.Context,*gorm.DB)(entitlement.Catalog,error)}
...
type SnapshotStore struct{db *gorm.DB;catalog SnapshotCatalogProjection;cache ports.SnapshotCache}
func NewSnapshotStore(db *gorm.DB,catalog SnapshotCatalogProjection,cache ports.SnapshotCache)(*SnapshotStore,error){if db==nil||catalog==nil{return nil,errors.New("commercial: snapshot authority required")};return &SnapshotStore{db:db,catalog:catalog,cache:cache},nil}
func cacheKey(tenant string,version uint64,digest string)string{return fmt.Sprintf("%s/%d/%s",tenant,version,digest)}
func(s *SnapshotStore)ReadSnapshot(ctx context.Context,tenant string,requested []string)(entitlement.Result,error){
 if tenant==""||len(tenant)>64||len(requested)>128{return entitlement.Result{},entitlement.ErrInvalid}
 frame,joined:=execution.Current(ctx)
 bounded,cancel:=context.WithTimeout(ctx,5*time.Second);defer cancel();ctx=bounded
 ...
 if err!=nil{return entitlement.Result{},err}
 ...
 if !joined&&s.cache!=nil{if b,err:=json.Marshal(result);err==nil{_ = s.cache.Put(ctx,cacheKey(tenant,result.EntitlementVersion,snapshot.Digest(b)),b)}}
...
func(s *SnapshotStore)locked(ctx context.Context,tx *gorm.DB,tenant string)(entitlement.Result,error){
 epoch,err:=consistency.LockCatalog(tx,false);if err!=nil{return entitlement.Result{},err}
 repo:=&entitlementRepository{tx:tx.Session(&gorm.Session{SkipDefaultTransaction:true})}
 state,err:=repo.Lock(ctx,tenant);if err!=nil{return entitlement.Result{},err}
 // Current locking reads defeat an earlier REPEATABLE READ image in this root.
 catalog,err:=s.catalog.ReadSnapshotCatalog(ctx,tx);if err!=nil{return entitlement.Result{},err}
 at,err:=consistency.Now(tx);if err!=nil{return entitlement.Result{},err}
 input,err:=snapshot.InputHash(catalog,state.Sources);if err!=nil{return entitlement.Result{},err}
 var head snapshotHead
 err=tx.Clauses(clause.Locking{Strength:"UPDATE"}).Where("tenant_id=?",tenant).First(&head).Error
 exists:=!errors.Is(err,gorm.ErrRecordNotFound);if err!=nil&&exists{return entitlement.Result{},err}
 ...
 history:=!errors.Is(err,gorm.ErrRecordNotFound);if err!=nil&&history{return entitlement.Result{},err}
 if exists!=history||(exists&&(head.Version!=latest.Version||state.Version<head.SourceVersion||epoch<head.CatalogRevision)){return entitlement.Result{},consistency.ErrStateLost}
 if exists&&!head.Invalidated&&head.SourceVersion==state.Version&&head.CatalogRevision==epoch&&head.InputHash==input{
  result,err:=s.decode(ctx,head,latest,at);if err==nil{return result,nil};if !errors.Is(err,snapshot.ErrStale){return entitlement.Result{},err}
 }
 if exists&&head.Version==math.MaxUint64{return entitlement.Result{},consistency.ErrStateLost}
 result,err:=entitlement.Resolve(tenant,state.Version,at,catalog,state.Sources,nil);if err!=nil{return entitlement.Result{},err}
 result.EntitlementVersion=head.Version+1;result.CatalogRevision=epoch
 b,err:=json.Marshal(result);if err!=nil{return entitlement.Result{},err};digest:=snapshot.Digest(b)
 row:=snapshotRow{TenantID:tenant,Version:result.EntitlementVersion,Payload:string(b),PayloadSHA256:digest,CreatedAt:at}
 if err:=tx.Create(&row).Error;err!=nil{return entitlement.Result{},err}
 ...
 if exists{
  ...
  if res.Error!=nil{return entitlement.Result{},res.Error};if res.RowsAffected!=1{return entitlement.Result{},consistency.ErrStateLost}
 }else if err:=tx.Create(&next).Error;err!=nil{return entitlement.Result{},err}
 return result,nil
}
func(s *SnapshotStore)decode(ctx context.Context,h snapshotHead,row snapshotRow,at time.Time)(entitlement.Result,error){
 if row.TenantID!=h.TenantID||row.Version!=h.Version||row.PayloadSHA256!=h.PayloadSHA256{return entitlement.Result{},snapshot.ErrInvalid}
 if s.cache!=nil{if b,err:=s.cache.Get(ctx,cacheKey(h.TenantID,h.Version,h.PayloadSHA256));err==nil&&len(b)>0{if r,err:=snapshot.Decode(b,h.TenantID,h.Version,h.SourceVersion,h.CatalogRevision,h.PayloadSHA256,at);err==nil{return r,nil}}}
...
func(s *SnapshotStore)readOnly(ctx context.Context,tenant string,requested []string)(entitlement.Result,error){
 db,err:=consistency.DB(ctx,nil);if err!=nil{return entitlement.Result{},err}
 var row struct{Version,SourceVersion,CatalogRevision,CurrentSource,CurrentCatalog uint64;Invalidated bool;PayloadSHA256,Payload string;At time.Time}
 ...
 if r.Error!=nil{return entitlement.Result{},r.Error};if r.RowsAffected!=1{return entitlement.Result{},snapshot.ErrStale}
 if row.Invalidated||row.SourceVersion!=row.CurrentSource||row.CatalogRevision!=row.CurrentCatalog{return entitlement.Result{},snapshot.ErrStale}
