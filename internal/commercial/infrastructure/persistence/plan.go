...
type planRefRow struct {
	PlanCode   string `gorm:"column:plan_code;primaryKey"`
	Version    uint64 `gorm:"primaryKey"`
	ModuleCode string `gorm:"column:module_code;primaryKey"`
}
...
func NewPlanRepositoryFactory() requestscope.RepositoryFactory[ports.PlanRepositories] {
	return requestscope.GORMRepositories(func(_ context.Context, tx *gorm.DB) (ports.PlanRepositories, error) {
		if tx == nil {
			return ports.PlanRepositories{}, errors.New("plans: root transaction required")
		}
		return ports.PlanRepositories{Plans: &planRepository{tx: tx.Session(&gorm.Session{SkipDefaultTransaction: true})}}, nil
	})
}
func (r *planRepository) Now(ctx context.Context) (time.Time, error) {
...
func (r *planRepository) Lock(ctx context.Context, code string) (ports.PlanHead, error) {
	...
	if err := db.Clauses(clause.OnConflict{DoNothing: true}).Create(&planRow{PlanCode: code}).Error; err != nil {
		return ports.PlanHead{}, err
	}
	var row planRow
	if err := db.Clauses(clause.Locking{Strength: "UPDATE"}).Where("plan_code=?", code).First(&row).Error; err != nil {
		return ports.PlanHead{}, err
	}
...
func decodePlan(row planVersionRow) (plan.Version, error) {
	...
	if v.PlanCode != row.PlanCode || v.Number != row.Version || v.Revision != row.Revision || v.State != row.State || v.ContentSHA256 != row.ContentSHA256 {
		return v, plan.ErrCorrupt
	}
...
func (r *planRepository) Get(ctx context.Context, code string, number uint64, current bool) (plan.Version, error) {
	...
	v, err := decodePlan(row)
	if err != nil {
		return v, err
	}
	if head.Revision < v.PlanRevision {
		return v, plan.ErrCorrupt
	}
	v.PlanRevision = head.Revision
	return v, nil
}
func (r *planRepository) List(ctx context.Context, code string, after uint64, limit int) ([]plan.Version, error) {
	...
	if err := r.tx.WithContext(ctx).Where("plan_code=? AND version>?", code, after).Order("version ASC").Limit(limit).Find(&rows).Error; err != nil {
		return nil, err
	}
...
func (r *planRepository) Receipt(ctx context.Context, code, key, hash string) (*plan.Version, error) {
	var row planReceiptRow
	err := r.tx.WithContext(ctx).Clauses(clause.Locking{Strength: "UPDATE"}).Where("plan_code=? AND request_id=?", code, key).First(&row).Error
	if errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, nil
	}
	if err != nil {
	...
	if row.Fingerprint != hash {
		return nil, plan.ErrRequestConflict
	}
	var out plan.Version
	if err := json.Unmarshal([]byte(row.Payload), &out); err != nil {
		return nil, plan.ErrCorrupt
	}
	if out.PlanCode != code {
		return nil, plan.ErrCorrupt
	}
	if err := out.Integrity(); err != nil {
		return nil, err
	}
	return &out, nil
}
func (r *planRepository) Save(ctx context.Context, head ports.PlanHead, before *plan.Version, v plan.Version) error {
	...
	if head.Code != v.PlanCode || head.Revision == ^uint64(0) || v.PlanRevision != head.Revision+1 {
		return plan.ErrConflict
	}
	if before != nil {
		if before.Revision == ^uint64(0) || v.Revision != before.Revision+1 {
			return plan.ErrConflict
		}
		if before.State != plan.Draft && !(before.State == plan.Published && v.State == plan.Retired && before.ContentSHA256 == v.ContentSHA256) {
			return plan.ErrImmutable
		}
	} else if head.Latest == ^uint64(0) || v.Number != head.Latest+1 || v.Revision != 1 || v.State != plan.Draft {
		return plan.ErrConflict
	}
	...
	res := db.Model(&planRow{}).Where("plan_code=? AND revision=?", head.Code, head.Revision).Updates(map[string]any{"revision": head.Revision + 1, "latest_version": latest})
	if res.Error != nil {
		return res.Error
	}
	...
	if before == nil || before.State == plan.Draft {
		if err := db.Where("plan_code=? AND version=?", v.PlanCode, v.Number).Delete(&planRefRow{}).Error; err != nil {
			return err
		}
...
func (r *planRepository) Complete(ctx context.Context, operation, key, hash string, before *plan.Version, after plan.Version, at time.Time) error {
	...
	db := r.tx.WithContext(ctx)
	if err := db.Create(&planAuditRow{PlanCode: after.PlanCode, Version: after.Number, RequestID: key, Action: operation, ActorID: after.ActorID, Reason: after.Reason, BeforeJSON: string(a), AfterJSON: string(b), CreatedAt: at}).Error; err != nil {
		return err
	}
