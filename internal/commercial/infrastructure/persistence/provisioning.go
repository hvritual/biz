...
func NewProvisioningRepositoryFactory() requestscope.RepositoryFactory[ports.ProvisioningRepositories] {
	return requestscope.GORMRepositories(func(_ context.Context, tx *gorm.DB) (ports.ProvisioningRepositories, error) {
		if tx == nil {
			return ports.ProvisioningRepositories{}, errors.New("provisioning: original root transaction required")
		}
...
func (r *provisioningRepository) Get(ctx context.Context, tenant, id string) (*p.Task, error) {
	return r.get(ctx, tenant, id, false)
}
...
func (r *provisioningRepository) List(ctx context.Context, tenant, after string, limit uint32) ([]p.Task, error) {
	...
	if e := r.tx.WithContext(ctx).Where("tenant_id=? AND task_id>?", tenant, after).Order("task_id").Limit(int(limit)).Find(&rows).Error; e != nil {
		return nil, e
	}
	out := []p.Task{}
...
func (r *provisioningRepository) Due(ctx context.Context, now time.Time, limit uint32) ([]p.Task, error) {
	...
	var rows []provisioningRow
	if e := r.tx.WithContext(ctx).Where("(state IN ? AND next_attempt_at<=?) OR (state=? AND lease_until<=?)", []string{p.Queued, p.RetryWait, p.Ready}, now, p.Running, now).Order("next_attempt_at,task_id").Limit(int(limit)).Find(&rows).Error; e != nil {
		return nil, e
	}
	out := []p.Task{}
	for _, row := range rows {
...
func taskRow(t p.Task) (provisioningRow, error) {
	if e := t.Integrity(); e != nil {
		return provisioningRow{}, e
	}
	b, e := json.Marshal(t)
...
func (r *provisioningRepository) Insert(ctx context.Context, t p.Task) error {
	...
	if e = r.tx.WithContext(ctx).Create(&row).Error; e != nil {
		return e
	}
...
func (r *provisioningRepository) Save(ctx context.Context, before, after p.Task, actor, reason string) error {
	if before.ID != after.ID || before.TenantID != after.TenantID || after.Revision != before.Revision+1 || p.Digest(before.Approval) != p.Digest(after.Approval) || !p.Reason(reason) || actor == "" {
...
func (r *provisioningRepository) ActionReceipt(ctx context.Context, actor, op, key, fp string) (*p.Task, error) {
	...
	if row.ActorID != actor || row.Operation != op || row.RequestID != key || row.Fingerprint != fp {
		return nil, p.ErrKeyConflict
	}
	var t p.Task
	if json.Unmarshal([]byte(row.Payload), &t) != nil || t.Integrity() != nil || t.Hash != row.PayloadSHA256 || t.ID != row.TaskID {
		return nil, p.ErrCorrupt
	}
	return &t, nil
}
