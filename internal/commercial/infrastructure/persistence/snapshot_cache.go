...
func NewMemorySnapshotCache(max int) *MemorySnapshotCache {
	if max < 1 {
		max = 1
	}
	return &MemorySnapshotCache{max: max, values: map[string][]byte{}}
}
...
func (c *MemorySnapshotCache) Put(_ context.Context, key string, b []byte) error {
	c.mu.Lock()
	defer c.mu.Unlock()
