...
func NewSubscriptionChangeRepositoryFactory() requestscope.RepositoryFactory[ports.SubscriptionChangeRepositories] {
 return requestscope.GORMRepositories(func(_ context.Context,tx *gorm.DB)(ports.SubscriptionChangeRepositories,error){
  if tx==nil{return ports.SubscriptionChangeRepositories{},errors.New("subscription changes: root transaction required")}
...
func (r *subscriptionChangeRepository) Now(ctx context.Context)(time.Time,error){return consistency.Now(r.tx.WithContext(ctx))}
func (r *subscriptionChangeRepository) LockTenant(ctx context.Context,tenant string)(subscription.Subscription,error){
 if _,err:=consistency.LockCatalog(r.tx.WithContext(ctx),false);err!=nil{return subscription.Subscription{},err}
...
func (r *subscriptionChangeRepository) Preview(ctx context.Context,tenant,id string,current bool)(*change.Preview,error){
 db:=r.tx.WithContext(ctx);if current{db=db.Clauses(clause.Locking{Strength:"SHARE"})}
 ...
 if json.Unmarshal([]byte(row.Payload),&p)!=nil||p.ChangeID!=row.ChangeID||p.Input.TenantID!=row.TenantID||p.ActorID!=row.ActorID||p.Input.RequestID!=row.RequestID||p.Fingerprint!=row.Fingerprint||p.Hash!=row.PayloadSHA256||!p.CreatedAt.Equal(row.CreatedAt)||!p.ExpiresAt.Equal(row.ExpiresAt)||p.Integrity()!=nil{return nil,change.ErrCorrupt}
 return &p,nil
}
func (r *subscriptionChangeRepository) SavePreview(ctx context.Context,p change.Preview)error{
 if err:=p.Integrity();err!=nil{return err};b,err:=json.Marshal(p);if err!=nil{return err}
 return r.tx.WithContext(ctx).Create(&changePreviewRow{ChangeID:p.ChangeID,TenantID:p.Input.TenantID,ActorID:p.ActorID,RequestID:p.Input.RequestID,Fingerprint:p.Fingerprint,PayloadSHA256:p.Hash,Payload:string(b),CreatedAt:p.CreatedAt,ExpiresAt:p.ExpiresAt}).Error
}
func decodeChangeReceipt(row changeReceiptRow)(*change.Receipt,error){
 var v change.Receipt
 if json.Unmarshal([]byte(row.Payload),&v)!=nil||v.ChangeID!=row.ChangeID||v.TenantID!=row.TenantID||v.ActorID!=row.ActorID||v.RequestID!=row.RequestID||v.Fingerprint!=row.Fingerprint||v.Hash!=row.PayloadSHA256||v.Status!=row.Status||!v.ConfirmedAt.Equal(row.ConfirmedAt)||v.Integrity()!=nil{return nil,change.ErrCorrupt}
 return &v,nil
}
func (r *subscriptionChangeRepository) Receipt(ctx context.Context,tenant,id string,current bool)(*change.Receipt,error){
 db:=r.tx.WithContext(ctx);if current{db=db.Clauses(clause.Locking{Strength:"SHARE"})}
 var row changeReceiptRow;err:=db.Where("tenant_id=? AND change_id=?",tenant,id).First(&row).Error
 if errors.Is(err,gorm.ErrRecordNotFound){return nil,nil};if err!=nil{return nil,err};return decodeChangeReceipt(row)
}
func (r *subscriptionChangeRepository) ReceiptForRequest(ctx context.Context,tenant,actor,key,fp string)(*change.Receipt,error){
 ...
 err:=r.tx.WithContext(ctx).Clauses(clause.Locking{Strength:"SHARE"}).Where("tenant_id=? AND actor_id=? AND request_id=?",tenant,actor,key).First(&row).Error
 if errors.Is(err,gorm.ErrRecordNotFound){return nil,nil};if err!=nil{return nil,err}
 v,err:=decodeChangeReceipt(row);if err!=nil{return nil,err};if v.Fingerprint!=fp{return nil,change.ErrRequestConflict};return v,nil
}
func (r *subscriptionChangeRepository) SaveCurrent(ctx context.Context,before,after subscription.Subscription)error{
 if before.TenantID!=after.TenantID||before.ID!=after.ID||before.Revision==0||before.Revision==^uint64(0)||after.Revision!=before.Revision+1||after.Validate()!=nil{return change.ErrConflict}
 b,err:=json.Marshal(after);if err!=nil{return err}
 ...
 if res.Error!=nil{return res.Error};if res.RowsAffected!=1{return change.ErrConflict};return nil
}
func (r *subscriptionChangeRepository) Complete(ctx context.Context,v change.Receipt)error{
 if err:=v.Integrity();err!=nil{return err};b,err:=json.Marshal(v);if err!=nil{return err}
 ...
 if err=r.tx.WithContext(ctx).Create(&row).Error;err!=nil{return err}
