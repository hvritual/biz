...
func (m ModulePolicy) Requirements(_ context.Context, v plan.Version) ([]p.Requirement, error) {
	...
	for _, module := range v.Terms.Modules {
		selected[module.Code] = true
	}
	...
	for _, module := range modules {
		if !plan.Code(module.ModuleCode) || seen[module.ModuleCode] {
			return nil, fmt.Errorf("provisioning: duplicate or invalid module policy")
		}
		seen[module.ModuleCode] = true
		if p.ValidateRequirements(module.Steps) != nil {
			return nil, p.ErrInvalid
		}
		if selected[module.ModuleCode] {
			out = append(out, module.Steps...)
		}
	}
	if p.ValidateRequirements(out) != nil {
		return nil, p.ErrInvalid
	}
	if len(out) == 0 {
		return nil, nil
