...
func (s *Service) ReadCurrentCatalog(ctx context.Context) (entitlement.Catalog,error) {
 if _,ok:=execution.Current(ctx);ok {
  modules,err:=s.ListInScope(ctx);if err!=nil{return nil,err}
  result:=make(entitlement.Catalog,0,len(modules))
  for _,m:=range modules {result=append(result,entitlement.ModuleDefinition{Code:m.Code,TechnicalStatus:string(m.TechnicalStatus),SalesStatus:string(m.SalesStatus),Version:m.Version,Capabilities:m.CapabilityCodes,QuotaKeys:m.QuotaSchemaKeys,FieldKeys:m.FieldPolicySchemaKeys,Dependencies:m.Dependencies})}
  return result,result.Validate()
 }
 var rows []moduleRow
 if err:=s.store.db.WithContext(ctx).Order("module_code").Find(&rows).Error; err!=nil { return nil,err }
 out:=make(entitlement.Catalog,0,len(rows))
 for _,row:=range rows {
  def,ok:=s.registry.Definition(row.Code); if !ok { return nil,ErrUnknownDefinition }
  status:=row.TechnicalStatus; if !def.ImplementationReady { status=string(TechnicalNotReady) }
