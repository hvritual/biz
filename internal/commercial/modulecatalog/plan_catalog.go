...
func(s *Service) ListPlanCatalogInScope(ctx context.Context)([]Module,error){
 ...
 return requestscope.JoinValue(ctx,factory,func(scope *requestscope.View[catalogReadRepositories])([]Module,error){
  ...
  if _,err:=consistency.LockCatalog(db,false);err!=nil{return nil,err}
  var rows []moduleRow
  if err:=db.Clauses(clause.Locking{Strength:"SHARE"}).Order("module_code").Find(&rows).Error;err!=nil{return nil,err}
  out:=make([]Module,0,len(rows));for _,r:=range rows{d,ok:=s.registry.Definition(r.Code);if !ok{return nil,ErrUnknownDefinition};m:=rowToModule(r,d);if !d.ImplementationReady{m.TechnicalStatus=TechnicalNotReady};out=append(out,m)}
  return out,nil
 })
}
