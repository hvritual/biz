...
func TestCE02DefinitionGraphValidation(t *testing.T){
	...
	if d,ok:=r.Definition("access-management");!ok||len(d.CapabilityCodes)==0||len(d.QuotaSchemaKeys)==0||len(d.FieldPolicySchemaKeys)==0{t.Fatalf("production definition incomplete: %#v",d)}
}
