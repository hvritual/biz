...
func (s *Service) ReadSnapshotCatalog(ctx context.Context, tx *gorm.DB) (entitlement.Catalog, error) {
	var rows []moduleRow
	if err := tx.WithContext(ctx).Clauses(clause.Locking{Strength: "SHARE"}).Order("module_code").Find(&rows).Error; err != nil {
		return nil, err
	}
...
func (s *Service) transact(ctx context.Context, fn func(*gorm.DB) error) error {
	return consistency.Within(ctx, s.store.db, 0, func(tx *gorm.DB) error {
		if _, err := consistency.LockCatalog(tx, true); err != nil {
			return err
		}
		return fn(tx)
	})
