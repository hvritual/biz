...
type ce02GrantResolver struct{}
func (ce02GrantResolver) ResolveGrants(_ context.Context, request authz.GrantRequest)([]authz.Grant,error){if request.Principal.TenantID!=""{return nil,nil};out:=make([]authz.Grant,0,len(request.Permissions));for _,p:=range request.Permissions{out=append(out,authz.Grant{Permission:p,RoleID:"platform-admin"})};return out,nil}
...
func TestCE02PlatformAPIRejectsTenantPrincipal(t *testing.T){
	...
	resolver:=Resolver();policy,ok:=resolver.ResolvePolicy(context.Background(),method);if !ok{t.Fatal("generated ModuleCatalog policy missing")};if policy.TenantRequired{t.Fatal("platform module operation must not be tenant-bound")}
