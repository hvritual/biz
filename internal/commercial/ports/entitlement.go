...
type EntitlementState struct { Version uint64; Sources []entitlement.Source }
type OverrideReceipt struct { Source entitlement.Source `json:"source"`; SourceVersion uint64 `json:"source_version"` }
type EntitlementAudit struct {
 TenantID string; ActorID string; Action string; Reason string; RequestID string
 BeforeVersion uint64; AfterVersion uint64; Before *entitlement.Source; After entitlement.Source; At time.Time
}
