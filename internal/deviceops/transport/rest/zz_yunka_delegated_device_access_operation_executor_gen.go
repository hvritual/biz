...
func RegisterDelegatedDeviceAccessOperationExecutor(mux *http.ServeMux, application application.DelegatedDeviceAccessApplication, executor operation.Executor) error {
	if mux == nil {
...
func writeDelegatedDeviceAccessOperationError(writer http.ResponseWriter, err error) {
...
func (handler *DelegatedDeviceAccessOperationHandler) handleOperationGetDelegatedDevice(writer http.ResponseWriter, request *http.Request) {
	wire := &deviceopsv1.GetDelegatedDeviceRequest{}
	wire.Id = request.PathValue("id")
	callContext := execution.WithIdempotencyKey(request.Context(), request.Header.Get("Idempotency-Key"))
	output, err := operation.ExecuteTyped(callContext, handler.executor, policy.OperationPlanDelegatedDeviceAccessGetDelegatedDevice(), wire, handler.application.GetDelegatedDevice)
	if err != nil {
		writeDelegatedDeviceAccessOperationError(writer, err)
		return
	}
...
func (handler *DelegatedDeviceAccessOperationHandler) handleOperationUpdateDelegatedDevice(writer http.ResponseWriter, request *http.Request) {
	wire := &deviceopsv1.UpdateDelegatedDeviceRequest{}
	...
	wire.Id = request.PathValue("id")
	callContext := execution.WithIdempotencyKey(request.Context(), request.Header.Get("Idempotency-Key"))
	output, err := operation.ExecuteTyped(callContext, handler.executor, policy.OperationPlanDelegatedDeviceAccessUpdateDelegatedDevice(), wire, handler.application.UpdateDelegatedDevice)
	if err != nil {
		writeDelegatedDeviceAccessOperationError(writer, err)
		return
	}
