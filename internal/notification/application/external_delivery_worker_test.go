...
func (r *externalRepoFake) ClaimNextExternalTask(context.Context, string, domain.ExternalDeliveryPolicy) (domain.ExternalTaskClaim, domain.ExternalTaskReceipt, error) {
	return r.claim, domain.ExternalTaskReceipt{TaskID: r.claim.TaskID, State: domain.ExternalTaskStateLeased, Attempt: r.claim.Attempt}, nil
}
func (r *externalRepoFake) CompleteExternalTask(_ context.Context, _ domain.ExternalTaskClaim, result domain.ExternalProviderResult) (domain.ExternalTaskReceipt, error) {
	r.completed = result
	...
	return domain.ExternalTaskReceipt{TaskID: r.claim.TaskID, State: state, Attempt: r.claim.Attempt, ProviderReceipt: result.ReceiptID}, nil
}
func (r *externalRepoFake) FailExternalTask(_ context.Context, _ domain.ExternalTaskClaim, code string, retry bool, _ domain.ExternalDeliveryPolicy) (domain.ExternalTaskReceipt, error) {
	r.failureCode = code
	r.retryable = &retry
	state := domain.ExternalTaskStateManualReview
	if retry {
		state = domain.ExternalTaskStateRetryWait
	}
	return domain.ExternalTaskReceipt{TaskID: r.claim.TaskID, State: state, Attempt: r.claim.Attempt, FailureCode: code}, nil
}
func (r *externalRepoFake) TerminalExternalTask(_ context.Context, _ domain.ExternalTaskClaim, state, code string) (domain.ExternalTaskReceipt, error) {
	r.terminal = state
	r.failureCode = code
	return domain.ExternalTaskReceipt{TaskID: r.claim.TaskID, State: state, Attempt: r.claim.Attempt, FailureCode: code}, nil
}
...
type admissionFake struct {
	admission accessdomain.OptionalNotificationDeliveryAdmission
	err       error
	calls     int
}

func (a *admissionFake) PrepareOptionalNotificationDelivery(context.Context, accessdomain.NotificationPreferenceOwner, accessdomain.NotificationPreferenceChannel) (accessdomain.OptionalNotificationDeliveryAdmission, error) {
	a.calls++
...
func (p *providerFake) SendExternalNotification(_ context.Context, request domain.ExternalProviderRequest) (domain.ExternalProviderResult, error) {
	p.calls++
	p.request = request
...
func externalWorkerFixture(t *testing.T, admission accessdomain.OptionalNotificationDeliveryAdmission, provider *providerFake) (*ExternalDeliveryWorker, *externalRepoFake, *admissionFake) {
	t.Helper()
	repo := &externalRepoFake{claim: externalClaimFixture()}
	admitter := &admissionFake{admission: admission}
	worker, err := NewExternalDeliveryWorker(ports.ExternalDeliveryDependencies{Tasks: repo, Admission: admitter, Provider: provider}, domain.EnterpriseExternalDeliveryPolicy(), "worker-185")
	if err != nil {
		t.Fatal(err)
	}
...
func TestEnterprise185ExternalWorkerRechecksPreferenceBeforeProvider(t *testing.T) {
	...
	if err != nil || admitter.calls != 1 || provider.calls != 0 || repo.terminal != domain.ExternalTaskStateCancelled || repo.failureCode != "PREFERENCE_DENIED" || receipt.State != domain.ExternalTaskStateCancelled {
		t.Fatalf("receipt=%+v admission=%d provider=%d terminal=%s code=%s err=%v", receipt, admitter.calls, provider.calls, repo.terminal, repo.failureCode, err)
	}
}
...
func TestEnterprise185ExternalWorkerUsesTransientContactAndProviderReceipt(t *testing.T) {
	...
	receipt, err := worker.RunOnce(context.Background())
	if err != nil || provider.calls != 1 || provider.request.Destination != "member@example.invalid" || repo.completed.ReceiptID != "provider-185" || receipt.State != domain.ExternalTaskStateProviderAccepted {
		t.Fatalf("receipt=%+v request=%+v completed=%+v err=%v", receipt, provider.request, repo.completed, err)
	}
}
...
func TestEnterprise185ExternalWorkerMissingContactIsManualReview(t *testing.T) {
	...
	receipt, err := worker.RunOnce(context.Background())
	if err != nil || provider.calls != 0 || repo.terminal != domain.ExternalTaskStateManualReview || repo.failureCode != "CONTACT_UNAVAILABLE" || receipt.State != domain.ExternalTaskStateManualReview {
...
func TestEnterprise185ExternalWorkerUnknownOutcomeRetriesOnlySafeProvider(t *testing.T) {
	for _, tc := range []struct {
		name            string
		safe, wantRetry bool
	}{{"unsafe", false, false}, {"safe", true, true}} {
		t.Run(tc.name, func(t *testing.T) {
			...
			worker, repo, _ := externalWorkerFixture(t, admission, provider)
			receipt, err := worker.RunOnce(context.Background())
			if err != nil || repo.retryable == nil || *repo.retryable != tc.wantRetry {
				t.Fatalf("receipt=%+v retry=%v err=%v", receipt, repo.retryable, err)
			}
			if tc.wantRetry && receipt.State != domain.ExternalTaskStateRetryWait {
				t.Fatalf("safe provider state=%s", receipt.State)
			}
			if !tc.wantRetry && receipt.State != domain.ExternalTaskStateManualReview {
				t.Fatalf("unsafe provider state=%s", receipt.State)
			}
		})
	}
}
...
func TestEnterprise185ExternalWorkerKnownTransientFailureRetries(t *testing.T) {
	...
	worker, repo, _ := externalWorkerFixture(t, admission, provider)
	receipt, err := worker.RunOnce(context.Background())
	if err != nil || repo.retryable == nil || !*repo.retryable || repo.failureCode != "PROVIDER_TEMPORARY" || receipt.State != domain.ExternalTaskStateRetryWait {
		t.Fatalf("receipt=%+v retry=%v code=%s err=%v", receipt, repo.retryable, repo.failureCode, err)
	}
}
...
func TestEnterprise185ExternalRecoveredUnknownOutcomeRequiresIdempotentProvider(t *testing.T) {
	for _, tc := range []struct {
		...
		safe      bool
		wantState string
	}{{"unsafe", false, domain.ExternalTaskStateManualReview}, {"safe", true, domain.ExternalTaskStateProviderAccepted}} {
		t.Run(tc.name, func(t *testing.T) {
			...
			receipt, err := worker.RunOnce(context.Background())
			if err != nil || receipt.State != tc.wantState {
				t.Fatalf("receipt=%+v err=%v", receipt, err)
			}
			if tc.safe && provider.calls != 1 {
				t.Fatalf("safe provider calls=%d", provider.calls)
			}
			if !tc.safe && (provider.calls != 0 || repo.failureCode != "DELIVERY_OUTCOME_UNKNOWN") {
				t.Fatalf("unsafe provider calls=%d code=%s", provider.calls, repo.failureCode)
			}
		})
	}
}
