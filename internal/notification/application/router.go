...
func (router *BusinessEventRouter) RouteOnce(ctx context.Context, workerID string) (domain.RoutingResult, error) {
	...
	if claim.Event.EventID == "" {
		return domain.RoutingResult{}, nil
	}
	event := claim.Event
	...
	for _, userID := range ids {
		...
		for _, code := range config.Channels {
			...
			if !allowed {
				outcome = domain.RouteOutcomePreferenceDenied
			}
...
func optionalPreferenceChannel(code string) (accessdomain.NotificationPreferenceChannel, bool) {
	switch code {
	case "sms":
		return accessdomain.NotificationPreferenceSMS, true
	case "email":
		return accessdomain.NotificationPreferenceEmail, true
	default:
		return "", false
	}
}
