...
func (r *workerRepo) ClaimNextSecurityNotification(context.Context, string, accessdomain.NotificationRetryPolicy) (accessdomain.ReliableSecurityNotificationClaim, accessdomain.NotificationDeliveryReceipt, error) {
	return r.claim, accessdomain.NotificationDeliveryReceipt{}, nil
}
func (r *workerRepo) CompleteReliableSecurityNotification(_ context.Context, _ accessdomain.ReliableSecurityNotificationClaim, _ string) (accessdomain.NotificationDeliveryReceipt, error) {
	r.completed = true
	return accessdomain.NotificationDeliveryReceipt{EventID: r.claim.EventID, State: accessdomain.NotificationStateDelivered, Attempt: r.claim.Attempt}, nil
}
func (r *workerRepo) FailReliableSecurityNotification(_ context.Context, _ accessdomain.ReliableSecurityNotificationClaim, code string, retry bool, _ accessdomain.NotificationRetryPolicy) (accessdomain.NotificationDeliveryReceipt, error) {
	r.failedRetryable = &retry
	r.failureCode = code
	state := accessdomain.NotificationStateManualReview
	if retry {
		state = accessdomain.NotificationStateRetryWait
	}
	return accessdomain.NotificationDeliveryReceipt{EventID: r.claim.EventID, State: state, FailureCode: code, Attempt: r.claim.Attempt}, nil
}
...
func (s workerSender) SendSecurityNotification(context.Context, accessdomain.SecurityNotificationClaim) (string, error) {
	if s.err != nil {
		return "", s.err
	}
...
func TestEnterprise185RetryPolicyIsBounded(t *testing.T) {
	...
	for i, d := range want {
		got, ok := p.NextDelay(uint32(i + 1))
		if !ok || got != d {
			t.Fatalf("attempt %d delay=%v ok=%v", i+1, got, ok)
		}
	}
	if _, ok := p.NextDelay(5); ok {
		t.Fatal("fifth failure retried")
	}
}
...
func TestEnterprise185WorkerCompletesDeliveredClaim(t *testing.T) {
	...
	if err != nil || !repo.completed || result.State != accessdomain.NotificationStateDelivered {
		t.Fatalf("result=%+v completed=%v err=%v", result, repo.completed, err)
	}
}
...
func TestEnterprise185KnownTransientFailureRetries(t *testing.T) {
	...
	result, err := w.RunOnce(context.Background())
	if err != nil || repo.failedRetryable == nil || !*repo.failedRetryable || result.State != accessdomain.NotificationStateRetryWait || repo.failureCode != "PROVIDER_REJECTED" {
		t.Fatalf("result=%+v retry=%v code=%s err=%v", result, repo.failedRetryable, repo.failureCode, err)
	}
}
...
func TestEnterprise185UnknownOutcomeRequiresIdempotentProvider(t *testing.T) {
	for _, tc := range []struct {
		...
		safe bool
		want bool
	}{{"unsafe", false, false}, {"idempotent", true, true}} {
		t.Run(tc.name, func(t *testing.T) {
			...
			result, err := w.RunOnce(context.Background())
			if err != nil || repo.failedRetryable == nil || *repo.failedRetryable != tc.want {
				t.Fatalf("result=%+v retry=%v err=%v", result, repo.failedRetryable, err)
			}
			if tc.want && result.State != accessdomain.NotificationStateRetryWait {
				t.Fatalf("expected retry: %+v", result)
			}
			if !tc.want && result.State != accessdomain.NotificationStateManualReview {
				t.Fatalf("expected manual review: %+v", result)
			}
		})
	}
}
...
func TestEnterprise185RecoveredUnknownOutcomeRequiresIdempotentProvider(t *testing.T) {
	for _, tc := range []struct {
		...
		safe      bool
		wantState string
	}{{"unsafe", false, accessdomain.NotificationStateManualReview}, {"idempotent", true, accessdomain.NotificationStateDelivered}} {
		t.Run(tc.name, func(t *testing.T) {
			claim := claimFixture()
			...
			result, err := w.RunOnce(context.Background())
			if err != nil || result.State != tc.wantState {
				t.Fatalf("result=%+v err=%v", result, err)
			}
			if tc.safe && !repo.completed {
				t.Fatal("idempotent provider was not resumed")
			}
			if !tc.safe && (repo.failedRetryable == nil || *repo.failedRetryable || repo.failureCode != "DELIVERY_OUTCOME_UNKNOWN") {
				t.Fatalf("unsafe recovered outcome was retried: retry=%v code=%s", repo.failedRetryable, repo.failureCode)
			}
		})
	}
}
