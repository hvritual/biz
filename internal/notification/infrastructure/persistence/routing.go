...
func NewEventPublisher(tx *gorm.DB) (*EventPublisher, error) {
	if tx == nil || tx.Statement == nil { return nil, domain.ErrRoutingUnavailable }
	if _, ok := tx.Statement.ConnPool.(gorm.TxCommitter); !ok { return nil, errors.New("notification: root transaction required for business event") }
...
func (p *EventPublisher) AppendBusinessEvent(ctx context.Context, event domain.BusinessEvent) error {
	digest, err := event.Digest(); if err != nil { return err }
	now, err := routingNow(ctx, p.tx); if err != nil { return err }
	...
	if err := p.tx.WithContext(ctx).Clauses(clause.Locking{Strength:"SHARE"}).Where("BINARY event_id=?",event.EventID).Take(&existing).Error; err != nil { return err }
	if existing.PayloadHash != digest { return domain.ErrRoutingConflict }
	return nil
}
...
func NewRoutingRepository(db *gorm.DB)(*RoutingRepository,error){ if db==nil{return nil,domain.ErrRoutingUnavailable};return &RoutingRepository{db:db},nil }
func MigrateRouting(ctx context.Context,db *gorm.DB)error{if db==nil{return domain.ErrRoutingUnavailable};return db.WithContext(ctx).AutoMigrate(&routingEventRecord{},&inAppRecord{},&externalTaskRecord{},&routeOutcomeRecord{})}
func routingNow(ctx context.Context,db *gorm.DB)(time.Time,error){var row struct{Now time.Time `gorm:"column:now"`};if err:=db.WithContext(ctx).Raw("SELECT UTC_TIMESTAMP(6) AS now").Scan(&row).Error;err!=nil{return time.Time{},err};if row.Now.IsZero(){return time.Time{},domain.ErrRoutingUnavailable};return row.Now.UTC(),nil}
func eventFromRecord(row routingEventRecord)domain.BusinessEvent{return domain.BusinessEvent{EventID:row.EventID,TenantID:row.TenantID,GroupID:row.GroupID,TypeCode:row.TypeCode,Level:domain.MessageLevel(row.Level),TraceID:row.TraceID,ReferenceKind:row.ReferenceKind,ReferenceID:row.ReferenceID,OccurredAt:row.OccurredAt.UTC()}}
...
func (r *RoutingRepository) ClaimNextBusinessEvent(ctx context.Context, worker string, lease time.Duration)(domain.RoutingClaim,error){
	if r==nil||r.db==nil||!domain.ValidConfigurationID(worker,96)||lease<5*time.Second||lease>5*time.Minute{return domain.RoutingClaim{},domain.ErrRoutingInvalid}
	var claim domain.RoutingClaim
	err:=r.db.WithContext(ctx).Transaction(func(tx *gorm.DB)error{
		now,err:=routingNow(ctx,tx);if err!=nil{return err}
		...
		if row.LeaseToken==^uint64(0){return domain.ErrRoutingUnavailable}
		row.Attempts++;row.LeaseToken++;until:=time.UnixMicro(now.Add(lease).UnixMicro()).UTC()
		if err:=tx.Model(&routingEventRecord{}).Where("event_id=?",row.EventID).Updates(map[string]any{"state":domain.RoutingStateLeased,"attempts":row.Attempts,"lease_owner":worker,"lease_token":row.LeaseToken,"lease_until":until,"updated_at":now}).Error;err!=nil{return err}
		claim=domain.RoutingClaim{Event:eventFromRecord(row),WorkerID:worker,LeaseToken:row.LeaseToken,LeaseUntil:until,Attempt:row.Attempts};return nil
	})
	return claim,err
}
...
func (r *RoutingRepository) lockedEvent(ctx context.Context,tx *gorm.DB,claim domain.RoutingClaim)(routingEventRecord,time.Time,error){
	if err:=claim.Validate();err!=nil{return routingEventRecord{},time.Time{},err}
	var row routingEventRecord
	if err:=tx.WithContext(ctx).Clauses(clause.Locking{Strength:"UPDATE"}).Where("event_id=?",claim.Event.EventID).Take(&row).Error;err!=nil{return row,time.Time{},err}
	now,err:=routingNow(ctx,tx);if err!=nil{return row,time.Time{},err}
	if row.State==domain.RoutingStateRouted{return row,now,nil}
	if row.State!=domain.RoutingStateLeased||row.LeaseOwner!=claim.WorkerID||row.LeaseToken!=claim.LeaseToken||row.LeaseUntil==nil||!now.Before(*row.LeaseUntil){return row,now,domain.ErrRoutingLease}
	digest,err:=claim.Event.Digest();if err!=nil||digest!=row.PayloadHash{return row,now,domain.ErrRoutingConflict}
	return row,now,nil
}
...
func (r *RoutingRepository) CompleteBusinessEventRoute(ctx context.Context,claim domain.RoutingClaim,plan domain.RoutePlan)(domain.RoutingResult,error){
	canonical,err:=plan.Canonical();if err!=nil{return domain.RoutingResult{},err}
	var result domain.RoutingResult
	err=r.db.WithContext(ctx).Transaction(func(tx *gorm.DB)error{
		row,now,err:=r.lockedEvent(ctx,tx,claim);if err!=nil{return err}
		if row.State==domain.RoutingStateRouted{var readErr error;result,readErr=r.routeResult(ctx,tx,row.EventID);return readErr}
		for _,d:=range canonical.Decisions{
			...
			if err:=tx.Clauses(clause.OnConflict{DoNothing:true}).Create(&outcome).Error;err!=nil{return err}
			switch d.Outcome{
		...
		routed:=now
		if err:=tx.Model(&routingEventRecord{}).Where("event_id=?",row.EventID).Updates(map[string]any{"state":domain.RoutingStateRouted,"lease_owner":"","lease_until":nil,"routed_at":routed,"updated_at":now}).Error;err!=nil{return err}
		result,err=r.routeResult(ctx,tx,row.EventID);return err
	})
	return result,err
}

func (r *RoutingRepository) routeResult(ctx context.Context,tx *gorm.DB,eventID string)(domain.RoutingResult,error){
	var row routingEventRecord;if err:=tx.WithContext(ctx).Where("event_id=?",eventID).Take(&row).Error;err!=nil{return domain.RoutingResult{},err}
	out:=domain.RoutingResult{EventID:eventID,State:row.State};if row.RoutedAt!=nil{out.RoutedAt=row.RoutedAt.UTC()}
	var counts []struct{Outcome string;Count uint64}
	if err:=tx.WithContext(ctx).Table("biz_notification_route_outcomes").Select("outcome,COUNT(*) AS count").Where("event_id=?",eventID).Group("outcome").Scan(&counts).Error;err!=nil{return out,err}
	for _,c:=range counts{switch c.Outcome{
	case domain.RouteOutcomeInAppCreated:out.InAppCreated+=c.Count
	case domain.RouteOutcomeExternalTask:out.ExternalTasks+=c.Count
	case domain.RouteOutcomePreferenceDenied:out.Denied+=c.Count
	case domain.RouteOutcomeChannelUnavailable:out.Unavailable+=c.Count
	case domain.RouteOutcomeRecipientInactive:out.Inactive+=c.Count
	case domain.RouteOutcomeNoConfiguration:out.NoConfiguration+=c.Count
	case domain.RouteOutcomeTypeUnavailable:out.TypeUnavailable+=c.Count
	}}
	return out,nil
}
...
func (r *RoutingRepository) FindRoutingConfiguration(ctx context.Context,tenant,group string,level domain.MessageLevel)(domain.Configuration,bool,error){
	if r==nil||r.db==nil||!domain.ValidConfigurationID(tenant,64)||!domain.ValidConfigurationID(group,64)||!level.Valid(){return domain.Configuration{},false,domain.ErrRoutingInvalid}
	...
	if errors.Is(err,gorm.ErrRecordNotFound){return domain.Configuration{},false,nil};if err!=nil{return domain.Configuration{},false,err}
	...
	if err:=r.db.WithContext(ctx).Model(&configurationChannelRecord{}).Where("BINARY tenant_id=? AND BINARY configuration_id=?",tenant,row.ID).Order("channel ASC").Pluck("channel",&c.Channels).Error;err!=nil{return domain.Configuration{},false,err}
	if err:=r.db.WithContext(ctx).Model(&configurationRecipientRecord{}).Where("BINARY tenant_id=? AND BINARY configuration_id=?",tenant,row.ID).Order("user_id ASC").Pluck("user_id",&c.AdditionalUserIDs).Error;err!=nil{return domain.Configuration{},false,err}
	if err:=c.Values().Validate();err!=nil{return domain.Configuration{},false,domain.ErrRoutingUnavailable}
	return c,true,nil
}
