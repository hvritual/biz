...
func NewHTTPHandler(service *Service, auth *Authenticator, database *gorm.DB) http.Handler {
	handler := &HTTPHandler{service: service, auth: auth, database: database, mux: http.NewServeMux()}
...
func (handler *HTTPHandler) requireAuth(next http.Handler) http.Handler {
...
func accessFrom(request *http.Request) (AccessPlan, bool) {
	plan, ok := request.Context().Value(accessContextKey{}).(AccessPlan)
	return plan, ok
}

func (handler *HTTPHandler) me(writer http.ResponseWriter, request *http.Request) {
	if request.Method != http.MethodGet {
		writer.WriteHeader(http.StatusMethodNotAllowed)
		return
	}
	plan, ok := accessFrom(request)
	if !ok {
		writeError(writer, ErrUnauthorized)
		return
	}
...
func (handler *HTTPHandler) devices(writer http.ResponseWriter, request *http.Request) {
	plan, ok := accessFrom(request)
	if !ok {
		writeError(writer, ErrUnauthorized)
		return
...
func (handler *HTTPHandler) device(writer http.ResponseWriter, request *http.Request) {
	plan, ok := accessFrom(request)
	if !ok {
		writeError(writer, ErrUnauthorized)
		return
