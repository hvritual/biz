...
run_suite focused go test -count=1 -json ./internal/commercial/... ./internal/architecture -run '^TestCE06'
run_suite race go test -race -count=1 -json ./internal/commercial/domain/snapshot ./internal/commercial/infrastructure/consistency ./internal/commercial/infrastructure/persistence
bash scripts/ci_commercial_mysql.sh wait ce06
run_suite mysql go test -count=1 -tags=integration -json ./integration -run '^TestCE06MySQL'
run_suite restart-before go test -count=1 -tags=integration -json ./integration -run '^TestCE06PersistenceBeforeRestart$'
{
...
run_suite restart-after go test -count=1 -tags=integration -json ./integration -run '^TestCE06PersistenceAfterRestart$'
...
summary['suites']=verify_output('ce06',out)
...
summary['delegates']=contract['gates']['ce06-qualification.yml']['delegates']
