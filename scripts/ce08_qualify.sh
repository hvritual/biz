...
go test -count=1 -tags=integration -json ./integration -run '^(TestB122TenantLifecycleRESTAndGRPCUseUnifiedExecutor|TestB122TenantLifecycleUsesRootMySQLUnitOfWork|TestB125TenantCreateBootstrapsOwnerInOneRootUoW|TestB125ChildFailureRollsBackTenantAndMemberAndAllowsIdempotentRetry|TestB126ConcurrentTenantCreateSameIdempotencyKeyCreatesOneBootstrapTree)$' | tee "$out/b12-bootstrap.jsonl"
...
task=next(t for t in json.loads(Path('docs/commercial-entitlements/tasks.json').read_text())['tasks'] if t['id']=='CE-08')
summary={
  'task_id':'CE-08',
