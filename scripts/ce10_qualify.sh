...
fast_container="ce10-fast-${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-1}"
restart_container="ce10-restart-${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-1}"
...
docker run \
  ...
  -e MYSQL_DATABASE=biz_ce10_mysql \
...
docker run \
  ...
  -e MYSQL_ROOT_PASSWORD=root \
  -e MYSQL_DATABASE=biz_ce10_restart \
...
docker exec "$fast_container" mysql -uroot -proot -e "
  CREATE DATABASE IF NOT EXISTS biz_ce10_lease CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
  CREATE DATABASE IF NOT EXISTS biz_ce10_mysql CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
  CREATE DATABASE IF NOT EXISTS biz_ce10_race CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
"
...
(
  run_suite focused go test -timeout=5m -count=1 -json \
    ./internal/commercial/... ./internal/bizruntime ./internal/architecture -run '^TestCE10'
) &
pids+=("$!"); names+=("focused")

(
  run_suite lease env \
    'YUNKA_TEST_MYSQL_DSN=root:root@tcp(127.0.0.1:3306)/biz_ce10_lease?charset=utf8mb4&parseTime=true&loc=UTC&multiStatements=true' \
    go test -timeout=2m -count=1 -tags=integration -json \
    ./internal/commercial/infrastructure/persistence -run '^TestCE10MySQL'
) &
pids+=("$!"); names+=("lease")

(
  run_suite mysql env \
    'YUNKA_TEST_MYSQL_DSN=root:root@tcp(127.0.0.1:3306)/biz_ce10_mysql?charset=utf8mb4&parseTime=true&loc=UTC&multiStatements=true' \
    go test -timeout=8m -count=1 -tags=integration -json ./integration -run '^TestCE10MySQL'
) &
pids+=("$!"); names+=("mysql")

(
  run_suite race env \
    'YUNKA_TEST_MYSQL_DSN=root:root@tcp(127.0.0.1:3306)/biz_ce10_race?charset=utf8mb4&parseTime=true&loc=UTC&multiStatements=true' \
    go test -timeout=8m -race -count=1 -tags=integration -json ./integration \
    -run '^TestCE10MySQLConcurrentRuntimesDoNotPrepareTwice$'
) &
pids+=("$!"); names+=("race")
...
(
  ...
  export YUNKA_TEST_MYSQL_DSN='root:root@tcp(127.0.0.1:3307)/biz_ce10_restart?charset=utf8mb4&parseTime=true&loc=UTC&multiStatements=true'
...
for i in "${!pids[@]}"; do
  if ! wait "${pids[$i]}"; then
    echo "CE10_STAGE_FAILURE=${names[$i]}" >&2
    stage_status=1
  fi
done
record_timing ce10-parallel-stage "$stage_started"
test "$stage_status" = 0
...
summary = {
    ...
    'delegated_full_gate_coverage': [
        ...
        'ce09-qualification.yml',
        'evolution-qualification.yml',
    ],
}

suites = ['focused','lease','mysql','race','restart-before','restart-after']
for name in suites:
