...
stop_one() {
  local name=$1 expected=$2 port=$3 pid_file="$state_dir/$name.pid" pid
  [[ -f "$pid_file" ]] || return 0
  pid=$(<"$pid_file")
...
(cd "$root/web"; BIZ_DEV_API_TARGET=http://127.0.0.1:18380 VITE_DATA_MODE=api node "$vite_entry" --host 127.0.0.1 --port 14183 >"$state_dir/web.log" 2>&1 & echo $! >"$state_dir/web.pid")
