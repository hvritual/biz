...
当前为 **明确标注的本地交互预览**，默认 `VITE_DATA_MODE=demo`。真实 API 模式尚未接入当前企业页面，因此这些页面持续读取和写入按租户隔离的模拟数据。测试邮箱使用 `example.com`；图示数字来自隔离示例仓库，不代表生产事实。
...
预览更改按租户保存到浏览器 localStorage，包含本地审计记录。切换租户清空页面筛选、选择与对话框，不会将旧企业数据带入新企业。存储失败会回滚本次内存修改。模拟数据不代表功能缺失：当前界面必须保持完整布局、导航、表格、表单、操作和加载／空／错误状态，不能以空白页、静态截图或删减交互替代。

**未完成当前企业页面的生产认证/后端联调。** 没有发送真实邀请或重置邮件，没有修改真实密码、套餐或租户安全策略，也没有生产防篡改审计。即使 `VITE_DATA_MODE=api`，尚未接入 API 的页面仍展示完整的 demo 界面；这是页面主动选择的隔离模拟数据，而不是接口失败后的静默回退。已接入 API 的页面仍须如实展示请求失败。前端权限约束用于交互演示，不能替代服务端鉴权。
