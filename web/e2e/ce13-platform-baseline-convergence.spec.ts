...
test('TestCE13PlatformCommercialReusesJoinedCoffeeLinkNavigation', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '平台商业' }).click()
  const drawer = page.getByRole('dialog', { name: '平台商业导航' })
  await expect(drawer).toBeVisible()
  await expect(drawer.getByRole('button', { name: '模块目录' })).toBeVisible()
  await expect(drawer.getByRole('button', { name: '套餐版本' })).toBeVisible()
  await expect(drawer.getByRole('button', { name: '租户权益' })).toBeVisible()
  await expect(drawer.getByRole('button', { name: '预览订阅变更' })).toBeVisible()
  ...
  await page.getByRole('button', { name: '平台商业' }).click()
  await drawer.getByRole('button', { name: '模块目录' }).click()
