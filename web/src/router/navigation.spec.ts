...
describe('primary navigation information architecture', () => {
  it('keeps the rail focused on business domains instead of page-level entries', () => {
    expect(primaryNavigation.map((item) => item.label)).toEqual([
      '工作台', '客户经营', '租赁运营', '设备运营', '经营管理', '企业中心', '平台管理', '系统设置',
    ])
  })
  ...
  it('separates semantic domain ids from stable automation selectors', () => {
    expect(primaryNavigation.filter((item) => item.selectorId).map((item) => [item.id, item.selectorId])).toEqual([
      ['customer-operations', 'customers'], ['rental-operations', 'sites'], ['device-operations', 'devices'], ['business-operations', 'orders'],
    ])
  })
  ...
  it('groups rental operations by lifecycle and exposes collection-level workspaces instead of sample entities', () => {
  ...
  it('places drink configuration under device operations rather than business operations', () => {
    expect(customerDomains['device-operations']?.links.map((item) => item.label)).toEqual(['设备管理', '饮品配置', '远程运维', '故障工单'])
    expect(groups('device-operations')).toEqual(['设备资产', '设备配置', '远程运维', '服务维护'])
    expect(customerDomains['business-operations']?.links.map((item) => item.label)).toEqual(['订单管理', '数据分析'])
  ...
  it('keeps system settings tenant-scoped and free of platform or enterprise shortcuts', () => {
    expect(systemNavigation.map((item) => item.label)).toEqual(['基础设置', '安全设置', '通知设置', '接口与集成', '数据字典'])
